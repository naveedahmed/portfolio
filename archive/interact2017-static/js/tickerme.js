...
(function($){
	...
	$.fn.tickerme = function(options) {
		...
		return this.each(function(){
			...
			var control_styles = '<style type="text/css">#ticker_container{width:100%}#newscontent{float:left}#news{display:none}#controls{height:16px}.icon{display:inline-block;width:16px;height:16px;fill:'+opts.control_colour+'}.icon:hover{fill:'+opts.control_rollover+'}</style>';
			...
			function init() {
				...
				controls += '<div id="newscontent"><div id="newsitem"></div></div>';
			...
			function load_container() {
				...
				if (opts.type == 'fade') {
					$('#news').fadeOut(opts.fade_speed,function(){
						$('#newscontent').html('<div id="newsitem">'+contents[position]+'</div>');
						$('#newsitem').fadeIn(opts.fade_speed);
					});
				}
			...
			$('a#prev_trigger').click(function(){
				...
				$('#newscontent').html('<div id="newsitem" style="display:block">'+contents[position]+'</div>');
